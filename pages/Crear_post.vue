  ...
  <div>
    <v-sheet elevation="12" class="ma-3 mt-3">
      ...
      <validation-observer v-slot="{ invalid, handleSubmit }">
        ...
        >
          <input type="hidden" name="depto" value="Principal"/>
          <v-card>
            <v-img
              class="white--text align-end"
              height="300px"
              src="/admin/admin_assets/blogBanner.jpg"
            ></v-img>
            <v-card-text>
              ...
              <validation-provider
                v-slot="{ errors }"
                rules="required"
                name="Imagen principal del post"
              >
                <v-file-input
              ...
              <div v-for="(grupoNum , i) in grupoItem" :key="i">
                ...
                <v-sheet elevation="12">
                  <v-card>
                    <v-card-title>Grupo de imagenes</v-card-title>
                    <v-card-text>
                      <v-textarea
                        :name="`GIDescripcion${i}`"
                        label="Descripcion corta"
                        clear-icon="mdi-broom"
                        hint="Se recomienda una descripcion menor a 50 caracteres"
                        counter
                        clearable
                        auto-grow
                        outlined
                      ...
                      <v-file-input
                        :name="`GIImg${i}[]`"
                        label="Seleccione las imagenes del grupo"
                        prepend-icon="mdi-camera"
                        accept="image/png, image/jpeg, image/bmp, image/jpg"
                        type="file"
                        multiple
              ...
              <v-row class="my-5">
                ...
                <v-col cols="4">
                  <v-btn color="blue darken-1" fab @click="addGrupoImagenes()">
                    <v-icon color="white" size="30px">mdi-plus</v-icon>
                  </v-btn>
                </v-col>
              </v-row>
            </v-card-text>
    ...
    async createPost() {
      ...
      try {
        ...
        let { data } = await this.$axios.post(
          "http://cruzrojatabasco.com/cruzrojatabasco/post/guardarPostSPA",
          formData,
          {
            headers: {
        ...
        this.showToastMessage(
          "A ocurrido un error durente la creacion del post",
          "mdi-timer-sand"
        );
        console.error(error);
      }
...
.caption_text {
  ...
  color: white;
  font-weight: 100;
  text-shadow: -2px -2px 3px $textColor, 2px -2px 3px $textColor,
    -2px 2px 3px $textColor, 2px 2px 3px $textColor;
}
</style>
